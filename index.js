...
const storeApps = async (clientGPS, result, no) => {
  for (const val of result) {
    const searchAppsData = getSearchAppsData(val.Body);
    for (const searchApp of searchAppsData) {
      try {
        ...
        const app = await getAppByAppId(clientGPS, searchApp);
        const naApp = await getNAAppByAppId(clientGPS, searchApp);
        if (app && naApp) {
        ...
        const primaryAppData = await getAppData(searchApp);

        if (Object.keys(primaryAppData).length === 0) {
          const naAppsMapping = await getNAAppsMapping(searchApp);
        ...
        const uniqueAppIds = uniqueApps.map((app) => app.appId);
...
const consumeSqsMessages = async (clientGPS, clientGPS1, clientGPS2, no) => {
  const params = {
    QueueUrl: SQS_URL,
